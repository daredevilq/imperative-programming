...
reserve(Vector *vector, size_t new_capacity) 
{
	if (new_capacity > vector->capacity)
	{
		...
		void *new_data_offset = (byte *)new_data + vector->capacity * vector->element_size;
...
resize(Vector *vector, size_t new_size) {
	...
	void *prev_end = (byte *)vector->data + (vector->size*vector->element_size);
#ifdef VECTOR_INIT_ZERO
	// clear new elements to zero
	memset(prev_end, 0, ((vector->capacity -vector->size)*vector->element_size));
...
push_back(Vector *vector, void *value) 
{
	...
	void *ins_point = (byte *)vector->data + (prev_size*vector->element_size);
...
insert(Vector *vector, size_t index, void *value)
{
	resize(vector, vector->size + 1);
	size_t prev_size = vector->size;
	void *ins_point = (byte *)vector->data + (index*vector->element_size);
	void *move_point = (byte *)ins_point + vector->element_size;
	size_t cpy_size = ((prev_size - index - 1)*vector->element_size);
...
{
	size_t prev_size = vector->size;
	void *move_point = (byte *)vector->data + (index*vector->element_size) ;
