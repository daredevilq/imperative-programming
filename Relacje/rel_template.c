...
{
	int refl=is_reflexive(relation,n);
...
int composition (pair *relation, int size, pair *relation_2, int size_2, pair *comp_relation)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		{	
			if (relation[i].second==relation_2[j].first)
			{
				...
				{
					if (comp_relation[k].first==relation[i].first && comp_relation[k].second==relation_2[j].second)
					{
						flag=0;
					}
				...
				if (flag==1)
				{
					comp_relation[counter].first=relation[i].first;
					comp_relation[counter].second=relation_2[j].second;
					counter++;
				}
			}
		}
	...
	{
		X[i]=comp_relation[temp].first;
		X[i+1]=comp_relation[temp].second;
        temp++;
